        ...
        <form id="bulkForm" method="POST" action="{{ route('admin.stories.bulk_destroy') }}">
            ...
            <div class="flex items-center justify-between gap-4 mb-4 flex-wrap pb-3 border-b border-slate-100">
                <h3 class="font-bold text-slate-800 flex items-center gap-2">
                    <span>💾</span> Saved Quizzes ({{ $collections->count() }})
                </h3>
                @if ($collections->isNotEmpty())
                    <div class="flex items-center gap-3">
                        <label class="flex items-center gap-1.5 text-xs font-semibold text-slate-600 cursor-pointer select-none">
                            <input type="checkbox" id="selectAll" class="rounded border-slate-300 accent-violet-600 w-4 h-4 cursor-pointer">
            ...
            @forelse ($collections as $c)
                <div class="flex items-center justify-between bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 mb-2.5 hover:border-violet-300 hover:bg-violet-50/20 transition gap-3">
                    <input type="checkbox" name="ids[]" value="{{ $c->id }}" class="bulk-item rounded border-slate-300 accent-violet-600 w-4 h-4 cursor-pointer">
                    <a href="{{ route('admin.quiz.show', $c) }}" class="flex-1 flex items-center justify-between gap-3 group">
                        <span class="font-bold text-slate-800 group-hover:text-violet-700 transition">🎯 {{ $c->title }}</span>
                        <span class="text-xs text-slate-500 font-medium">{{ $c->parts_count }} cards · {{ ucfirst($c->status) }}</span>
                    </a>
                    <button type="button" onclick="if(confirm('Kya aap sach me ye quiz collection delete karna chahte hain?')) document.getElementById('single-del-{{ $c->id }}').submit();" class="text-xs font-semibold text-red-600 hover:text-red-700 bg-red-50 hover:bg-red-100 border border-red-200 px-3 py-1.5 rounded-lg transition flex items-center gap-1">
                        <span>🗑</span> Delete
                    </button>
                </div>
            @empty
                <p class="text-sm text-slate-500 text-center py-4">Abhi koi quiz nahi — upar se pehla banao. 🎯</p>
            @endforelse
        </form>

        {{-- Hidden Single Delete Forms --}}
        @foreach ($collections as $c)
            <form id="single-del-{{ $c->id }}" method="POST" action="{{ route('admin.quiz.destroy', $c) }}" class="hidden">
                @csrf @method('DELETE')
            </form>
        @endforeach
    </div>
    ...
    if (selectAll && form) {
        ...
        if (bulkBtn) {
            bulkBtn.addEventListener('click', () => {
                ...
                if (count > 0 && confirm(`Kya aap sach me chune hue ${count} quizzes ko delete karna chahte hain?`)) {
