...
<div class="max-w-5xl">
    ...
    <div class="mt-8 bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
        <form id="bulkForm" method="POST" action="{{ route('admin.stories.bulk_destroy') }}">
            ...
            @forelse ($collections as $c)
                <div class="flex items-center justify-between bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 mb-2.5 hover:border-violet-300 hover:bg-violet-50/20 transition gap-3">
                    ...
                    <a href="{{ route('admin.studio.show', $c) }}" class="flex-1 flex items-center justify-between gap-3 group">
                        <span class="font-bold text-slate-800 group-hover:text-violet-700 transition">
                    ...
                    <button type="button" onclick="if(confirm('Kya aap sach me ye collection delete karna chahte hain?')) document.getElementById('single-del-{{ $c->id }}').submit();" class="text-xs font-semibold text-red-600 hover:text-red-700 bg-red-50 hover:bg-red-100 border border-red-200 px-3 py-1.5 rounded-lg transition flex items-center gap-1">
                        <span>🗑</span> Delete
                    </button>
                </div>
            @empty
        ...
        @foreach ($collections as $c)
            <form id="single-del-{{ $c->id }}" method="POST" action="{{ route('admin.studio.destroy', $c) }}" class="hidden">
                @csrf @method('DELETE')
            </form>
        @endforeach
    </div>
    ...
    if (selectAll && form) {
        ...
        if (bulkBtn) {
            bulkBtn.addEventListener('click', () => {
                ...
                if (count > 0 && confirm(`Kya aap sach me chune hue ${count} collections ko delete karna chahte hain?`)) {
