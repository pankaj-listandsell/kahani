...
@section('content')
<div class="max-w-5xl">
    <a href="{{ route('admin.this_or_that.index') }}" class="text-sm text-slate-500 hover:text-sky-700">← ⚖️ This or That Studio</a>

    <div class="flex items-start justify-between gap-4 mt-2 mb-6 flex-wrap">
        <div>
            <h2 class="text-xl font-bold">⚖️ {{ $story->title }}</h2>
            <p class="text-slate-500 mt-1 text-sm">{{ $cards->count() }} poll cards · {{ ucfirst($story->status) }}</p>
        </div>
        <form method="POST" action="{{ route('admin.this_or_that.destroy', $story) }}"
              onsubmit="return confirm('Poori poll collection delete karein?')">
            @csrf @method('DELETE')
    ...
    @else
        <form id="bulkCardsForm" method="POST" action="{{ route('admin.cards.bulk_destroy') }}">
            ...
            <div class="flex items-center justify-between gap-3 mb-4 bg-white border border-slate-200 rounded-xl p-3 shadow-sm flex-wrap">
                <label class="flex items-center gap-2 text-xs font-bold text-slate-700 cursor-pointer select-none">
                    <input type="checkbox" id="selectAllCards" class="rounded border-slate-300 accent-teal-600 w-4 h-4 cursor-pointer">
                    ...
                    <div class="bg-white rounded-xl border border-slate-200 p-2 shadow-sm relative">
                        <div class="flex items-center justify-between mb-1">
                            <label class="flex items-center gap-1 cursor-pointer">
                                <input type="checkbox" name="ids[]" value="{{ $card->id }}" class="bulk-card rounded border-slate-300 accent-teal-600 w-4 h-4 cursor-pointer">
                                <span class="text-[11px] font-bold text-teal-600">⚖️ Poll {{ $i + 1 }}</span>
                            </label>
                        </div>
                        ...
                        <button type="button"
                                class="gen-reel mt-2 w-full text-[11px] bg-teal-600 hover:bg-teal-700 text-white font-bold rounded px-2 py-1.5"
                                data-url="{{ route('admin.cards.reel', $card) }}">▶ Generate Reel</button>
                        <a href="{{ asset('storage/' . $card->image_path) }}" download
                           class="block text-center text-[11px] text-teal-600 hover:underline mt-1 font-medium">⬇ Image</a>
        ...
        <p class="text-xs text-slate-400 mt-4">Har card ek "This or That" debate poll hai. Auto-post inhe drip post karta hai.</p>
    @endif
</div>
